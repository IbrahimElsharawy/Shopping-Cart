...
const Register = () => {
  ...
  return (
    <>
      <button
        className="block text-blue-500 border-2 border-blue-500 bg-white mx-2  hover:bg-blue-500 hover:text-white transition duration-500	 ease-in-out flex  items-center justify-center  font-medium rounded-lg text-sm px-5 py-2.5 text-center "
        type="button"
        onClick={() => setShowModal(true)}
      >
        <FaUserPlus className="mr-1" size={16} />
        Register
      </button>
      {showModal ? (
        <>
          <div className="overflow-x-hidden overflow-y-auto fixed top-0 left-0 w-full right-0 z-50 outline-none focus:outline-none">
